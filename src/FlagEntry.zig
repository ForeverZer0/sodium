...
/// Allocates and initializes a new `FlagEntry` with the specified configuration.
///
...
pub fn create(allocator: Allocator, name: []const u8, shorthand: ?u8, usage: []const u8, value: AnyValue) Error!*FlagEntry {
    ...
    const flag = try allocator.create(FlagEntry);
    flag.* = .{
        .name = flag_name,
        .usage = flag_usage,
        .aliases = .{},
...
pub fn dupe(self: *const FlagEntry, allocator: Allocator) Allocator.Error!*FlagEntry {
    var flag = try allocator.create(FlagEntry);
...
pub fn destroy(self: *FlagEntry, allocator: Allocator) void {
...
pub fn argName(self: *FlagEntry) []const u8 {
